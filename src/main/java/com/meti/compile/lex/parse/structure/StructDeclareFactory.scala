...
object StructDeclareFactory extends TokenizerFactory {
  ...
  class TokenizerImpl(val content: String) extends Tokenizer {
    ...
    private def createBuilder: Builder = Monad(content)
      .extract((value: String) => value.indexOf('<'))
